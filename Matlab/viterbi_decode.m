...
%              g= [133 171 165] from standard , eliminated from the code as we need it only one time.
%                         array in octal
% Outputs:     out      - Ouput bit array

function [out] = viterbi_decode(in)
...
% tf_state = zeros(num_states, (length(in)/r)+1); %% 64 states * (7+1) stages , This optimization will be done in the C code
path_metric = zeros(num_states, (length(in)/r)+1); %% bug release
for(n=0:(length(in)/r)-1) % in the i_th time (stage of trellis)
    ...
    w_metric  = zeros(num_states, 2); % for not sending 1,0 pair (soft decision) , in our code it does nothing
    
    for(m=0:num_states-1) % in the immediate state with index m
        % Calculate the accumulated branch metrics (cost of each link pair pointing to the current state) for each state
        ...
        % add the branch cost to the total path cost until the current state for each of the 2 pair of links pointing
        % to the current state
        tmp1 = br_metric(m+1,1) + p_metric(m+1,1);
        tmp2 = br_metric(m+1,2) + p_metric(m+1,2);
        % compare the two branches and choosing the minimum cost path
        if(tmp1 > tmp2)
...
for(n=(length(in)/r)-1:-1:0)
    % deciding the input bit depending on the position of the state
    if((tb_state(1,idx-1) < (num_states/2)))
        out_back(1,idx-1) = 0;
    else
        out_back(1,idx-1) = 1;
    end
...
% flipping the output got from tracing back the path
out = fliplr(out_back);
