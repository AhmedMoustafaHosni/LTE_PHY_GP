...
% Outputs:     subframe              - the subframes for each antenna with 3D matrix
%                                      with dim (M_pusch_sc_ * 14 (M_symbs) * P)
% edit 27/1/2017
%by Ahmed Moustafa
	 ...
	 for L = 0:(N_symbs_per_slot*2)-1
		 if(3 == L)
			 for k=0:M_pusch_sc-1
				 % DMRS
				 subframe(k+1,L+1,P) = dmrs_1(P,k+1);
			 end
		 elseif(10 == L)
			 for k=0:M_pusch_sc-1
				 % DMRS
				 subframe(k+1,L+1,P) = dmrs_2(P,k+1);
			 end
		 else
			 for k=0:M_pusch_sc-1
				 % PUSCH
