...
%%
function [data_bits, ri_bits, ack_bits] = channel_deinterleaver_MIMO(in_bits, N_ri_bits, N_ack_bits, N_l, Q_m)

    %Transform the N_layer data to 1 row
    in_bits = reshape(in_bits,length(in_bits)*N_l,1).';
...
%     H_prime_total = H_prime + N_ri_bits;              %we edited this line
    ...
    while(k < H_prime)
        ...
        n = n + 1;
            ...
            data_bits = reshape(data_bits.',size(data_bits,1)*Q_m*N_l,1).';
            ri_bits = reshape(ri_bits.',size(ri_bits,1)*Q_m*N_l,1).';
end
