...
bits1 = (fread(fopen('D:\GP\Axxcelera_4G\CUDA\cuda_optimized\cuda3\cuda1\1.txt')) - '0').';
bits2 = (fread(fopen('D:\GP\Axxcelera_4G\CUDA\cuda_optimized\cuda3\cuda1\2.txt')) - '0').';

%ri_bits = (fread(fopen('D:\GP\Axxcelera_4G\CUDA\cuda_optimized\cuda2\cuda1\ri_0.txt'))-'0').'; 
...
M_pusch_rb = RBs;      %% number of resource blocks assigned to the UE
...
% input : 2 rows
% output : 2 rows
modulated_symbols1 = mapper(scrambled(1,:), '64qam');
modulated_symbols2 = mapper(scrambled(2,:), '64qam');
...
% %tx = fft(tx_signal(1,:));
% 
sum((abs(tx_signal(1,:)) - abs(symbols_CUDA)))
stop;
SNR = [-20:0];
for i = 1 : length(SNR)
	N0 = 0.5 / (10^(SNR(i)/10));
