...
%     txSignalWithNoise = txTimeDomainSig + noise;
	  txSignalWithNoise = awgn(txTimeDomainSig, SNR(i),'measured');
%% *Receiver*
%% *5. SC-FDMA Demodulation*
	...
	for L = 1 : N_SCFDMA
		...
		%%% making a hard decision in receiver, then convert it to soft bits for the next functions (they work on soft bits only)
