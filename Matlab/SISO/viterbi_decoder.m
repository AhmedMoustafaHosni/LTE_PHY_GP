...
function [decodded_output] = viterbi_decoder(in)
k= 7;
r = 3;
...
%% note that in this step we won't use the input array (in), as we will generate the transistion state as stated in MIT lect7
st_output = [  0,  7;
	4,  3;
	1,  6;
	5,  2;
	...
	4,  3;
	0,  7 ];
% the previous block is to precalculate the transition output from each state at any stage (time i)

%% prepare input to viterbi 
temp1 = reshape(in,length(in)/r,r)';
temp2 = reshape(temp1,length(in),1)';
in = temp2;
% Calculate branch and path metrics
path_metric = zeros(num_states, (length(in)/r)+1); 
