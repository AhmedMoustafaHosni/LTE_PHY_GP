...
void sc_fdma_modulator(cufftComplex* subframe_d, const int M_pusch_rb, cufftComplex** pusch_bb_d, cufftHandle plan_sc_fdma, cufftComplex* ifft_vec_d)
{
	...
	dim3 block(1024,1,1);
	reshape_ifft_vec <<< grid, block , 0, stream_default>>>(subframe_d, ifft_vec_d, M_pusch_sc/2);
	...
	cufftSetStream(plan_sc_fdma, stream_default);
	...
	add_cyclic_prefix << < grid1, block1 , 0, stream_default>> >(ifft_vec_d, *pusch_bb_d, M_pusch_sc);
