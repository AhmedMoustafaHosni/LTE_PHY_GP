...
void generate_ul_rs(int N_s, int N_id_cell, char* chan_type, int delta_ss, bool group_hopping_enabled, bool sequence_hopping_enabled, float alpha, int N_prbs, cufftComplex** ref_signal_h)
{
	//Calculate M_sc_rs
	...
	//Device data
	cufftComplex* x_q_d;
	cufftComplex* ref_signal_d;
	char* PHI_1_d;
	char* PHI_2_d;

	//Host data allocation
	*ref_signal_h = (cufftComplex *)malloc(sizeof(cufftComplex)*M_sc_rs);

	//Device data allocation
	startTimer();
	cudaMalloc((void **)&x_q_d, sizeof(cufftComplex)*N_zc_rs);
	cudaMalloc((void **)&ref_signal_d, sizeof(cufftComplex)*M_sc_rs);
	cudaMalloc((void **)&PHI_1_d, sizeof(cufftComplex)*360);
	cudaMalloc((void **)&PHI_2_d, sizeof(cufftComplex)*720);
	stopTimer("cudaMalloc Time= %.6f ms\n", elapsed);

	//Copying data to device
	startTimer();
	cudaMemcpy(PHI_1_d, PHI_1, sizeof(cufftComplex)*360, cudaMemcpyHostToDevice);
	cudaMemcpy(PHI_2_d, PHI_2, sizeof(cufftComplex) * 720, cudaMemcpyHostToDevice);
	...
	startTimer();
	cudaMemcpy(*ref_signal_h, ref_signal_d, sizeof(cufftComplex)*M_sc_rs, cudaMemcpyDeviceToHost);
	stopTimer("cudaMemcpy Device->Host Time= %.6f ms\n", elapsed);

	// Cleanup
	cudaFree(ref_signal_d);
	cudaFree(x_q_d);
	cudaFree(PHI_1_d);
	cudaFree(PHI_2_d);
