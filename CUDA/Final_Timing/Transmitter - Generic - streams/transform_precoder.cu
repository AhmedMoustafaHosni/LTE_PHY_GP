...
void transform_precoder(float* symbols_R_d,float* symbols_I_d, const int M_pusch_rb, int signal_size, cufftComplex** precoded_data_d, cufftHandle plan_transform_precoder, cufftComplex* symbols_d, cudaStream_t stream)
{
	...
	toComplex << <gridDim, blockDim  ,0, stream >> > (symbols_R_d, symbols_I_d, symbols_d, rsqrtf(M_pusch_sc), numThreads);
	cufftSetStream(plan_transform_precoder, stream);
