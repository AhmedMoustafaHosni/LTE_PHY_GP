...
void generate_ul_rs(int N_s, int N_id_cell, char* chan_type, int delta_ss, bool group_hopping_enabled, bool sequence_hopping_enabled, float alpha, int N_prbs, cufftComplex** ref_signal_d, cufftComplex* x_q_d, cudaStream_t stream)
{
	//Calculate M_sc_rs
	...
	if (M_sc_rs >= 3 * N_sc_rb)
	{
		...
		calculate_x_q << < 2 , 1024  ,0, stream >> >(q, N_zc_rs, x_q_d);
		calculate_ref_sig_case1 << < 2, 1024  ,0, stream >> >(x_q_d, N_zc_rs, alpha, M_sc_rs, *ref_signal_d);
	}
	else if (M_sc_rs == N_sc_rb)
	{
		calculate_ref_sig_case2 << < 2, 1024  ,0, stream >> >(u, alpha, M_sc_rs, PHI_1_d, *ref_signal_d);
	}
	else
	{
		calculate_ref_sig_case3 << < 2, 1024  ,0, stream >> >(u, alpha, M_sc_rs, PHI_2_d, *ref_signal_d);
	}
