...
//__global__ void test(cufftComplex* pusch_bb_d)
//{
//	int x = 0;
//}
...
void sc_fdma_modulator(cufftComplex* subframe_d, const int M_pusch_rb, cufftComplex** pusch_bb_d, cufftHandle plan_sc_fdma, cufftComplex* ifft_vec_d, cudaStream_t stream, int i)
{
	...
	dim3 block(1024,1,1);
	reshape_ifft_vec <<< grid, block,0 , stream >>>(subframe_d, ifft_vec_d, M_pusch_sc/2);
	...
	cufftSetStream(plan_sc_fdma, stream);
	...
	add_cyclic_prefix << < grid1, block1  ,0, stream >> >(ifft_vec_d, *pusch_bb_d+i*30720, M_pusch_sc);
	//test << <1, 1 >> > (*pusch_bb_d);
}
