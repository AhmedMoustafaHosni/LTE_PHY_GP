...
__global__ void serialOut(Byte* input_d, Byte* input_d2, Byte* input_d3, Byte* input_d4, Byte* y_mat_d, const int N, int Nrows, int Qm, int N_l) {
	...
	if (idx <( N / N_l))
		...
		y_mat_d[y*C_mux*Qm*N_l + z*Qm*N_l + x] = input_d3[idx - (2 * N / N_l)];
	else
		y_mat_d[y*C_mux*Qm*N_l + z*Qm*N_l + x] = input_d4[idx - (3 * N / N_l)];
	//printf("y_mat_d[%d] = %d\n", y*C_mux*Qm*N_l + z*Qm*N_l + x, y_mat_d[y*C_mux*Qm*N_l + z*Qm*N_l + x]);

}
void deinterleaver(Byte* input_d, Byte* input_d2, Byte* input_d3, Byte* input_d4, Byte** ri_d, Byte** output_d, const int N, const int N_ri, const int Qm, const int N_l, Byte* y_idx_d, Byte* y_mat_d)
{
	// Step 1: Define C_mux
	...
	serialOut << <gridDim_3, blockDim_3 >> >(input_d, input_d2, input_d3, input_d4, y_mat_d, N, R_prime_mux, Qm, N_l);
