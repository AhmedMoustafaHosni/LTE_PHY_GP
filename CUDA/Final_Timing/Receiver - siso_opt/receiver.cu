...
int main(int argc, char **argv) {
	...
	{
		subframe_h[i].x = rand() / (float)RAND_MAX * 0.01 ;
		subframe_h[i].y = rand() / (float)RAND_MAX * 0.01;
	}
	...
	//predecoding   
	transform_predecoder(demod_subframe_d, &predecoded_data_d, plan_transform_predecoder); //signal_size = 12 * M_pusch_sc
